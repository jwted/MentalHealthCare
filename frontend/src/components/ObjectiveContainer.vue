<template>
  <v-container
    class="d-flex align-center justify-center cont bg"
    v-if="obj && !startedObj"
  >
  ...
  >
    ...
    <v-col class="d-flex flex-column justify-center align-center bg" v-if="startedObj.state!='Finished'">
      <Button
        :text="'Detail'"
        class="dark mt-4 mb-4"
        @click="showDetail(startedObj.objectiveId)"
      ></Button>
      <Button
        :text="'Cancel'"
        class="dark"
        @click="deleteUserObjective(startedObj.objectiveId)"
      ></Button>
    </v-col>
